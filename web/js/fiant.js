...
try {
	...
	ux.addEventListener('filed-annotation', function (event) {
		if (event.target_object === ux.getAttribute("deep-id")) {
			renderObject()
		}
	})
	...
	async function render(obj = {}) {
		...
		switch (type) {
			case "sc:Canvas":
				...
				ux.setAttribute("deep-id", obj["@id"])
			...
			case "sc:Manifest":
				...
				ux.setAttribute("deep-id", SCREEN.canvas["@id"] || SCREEN.canvas.id || "")
	...
	function renderList(list) {
		...
		<ul style="list-style: outside none; padding:0;">
			...
			<a href="#${b.id||b["@id"]}">${setIcon(b)} ${b.name || b.label || b["@type"] }</a>
			</li>
			`,``)}
			</ul>`
		let update = (function(){
	...
	async function renderObject(object) {
		if (!object || (typeof object === "object")) {
			object = await fetch(ux.getAttribute("deep-id")).then(response => response.json())
		}
		...
		ux.innerHTML = tmplData
		// ux.getElementsByTagName("form")[0].onsubmit = saveAnnotations
		// dirtyWatch(ux.querySelectorAll("input, textarea"))
	}
...
function createPerson(event) {
	...
	newFolk.innerHTML = pform_name.value
